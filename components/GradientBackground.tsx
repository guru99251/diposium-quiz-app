...
export default function GradientBackground() {
  ...
  useEffect(() => {
    ...
    const isLowPowerDevice = isMobile || (navigator.hardwareConcurrency ?? 0) <= 4
    const basePixelRatio = window.devicePixelRatio || 1
    const cappedPixelRatio = Math.min(basePixelRatio, isMobile ? 1.5 : 2)
    ...
    const uniforms: Record<string, THREE.IUniform> = {
      ...
      uActualResolution: { value: new THREE.Vector2(window.innerWidth * cappedPixelRatio, window.innerHeight * cappedPixelRatio) },
      ...
      uBackgroundColor: { value: settings.backgroundColor.clone() },
      uSphereColor: { value: settings.sphereColor.clone() },
      uLightColor: { value: settings.lightColor.clone() },
      uLightPosition: { value: settings.lightPosition.clone() },
      ...
      uCursorGlowColor: { value: settings.cursorGlowColor.clone() },
    ...
    const precisionQualifier = isMobile || isSafari || isLowPowerDevice ? "precision mediump float;" : "precision highp float;"
    ...
    const cursorSphere = uniforms.uCursorSphere.value as THREE.Vector3
    const mousePosition = uniforms.uMousePosition.value as THREE.Vector2
    ...
    const updateCursor = (clientX: number, clientY: number) => {
      ...
      targetMousePosition.set(clientX / width, 1 - clientY / height)

      const worldPos = screenToWorld(targetMousePosition.x, targetMousePosition.y)
      cursorSphere.copy(worldPos)
      ...
      for (const pos of fixedPositions) {
        const dist = cursorSphere.distanceTo(pos)
      ...
      uniforms.uCursorSphere.value.copy(cursorSphere)
      uniforms.uCursorRadius.value = dynamicRadius
    }
    ...
    const handleResize = () => {
      ...
      ;(uniforms.uActualResolution.value as THREE.Vector2).set(
    ...
    const renderFrame = () => {
      ...
      uniforms.uMousePosition.value = mousePosition
  ...
  return <div className="metaball-background" ref={containerRef} aria-hidden suppressHydrationWarning />
}
